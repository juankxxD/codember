...
const validatedKey = (key) => {
    const [politics, word] = key.split(':');
    const [conditionalNumber, letter] = politics.split(' ')
    const [minAppearances, maxAppearances] =  conditionalNumber.split('-');

    const arrayWord = word.split('').filter((value) => value === letter);

    if(arrayWord.length >= Number(minAppearances) && arrayWord.length <= Number(maxAppearances)) {
        KeyCorrect.push(word.trim())
    } else {
        KeyIncorrect.push(word.trim());
    }
}
listKey.forEach((key) => validatedKey(key))

console.log(KeyIncorrect[41]);
